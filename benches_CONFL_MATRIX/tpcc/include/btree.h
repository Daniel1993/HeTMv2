...
public:
  ...
  __attribute__((transaction_safe)) void tm_insert(TM_ARGDECL KEY key, VALUE value) {
    //GCC warns that this may be used uninitialized, even though that is untrue.
    InsertionResult result = {KEY(), 0, 0};
    bool was_split;
    ...
    if (d == 0) {
      ...
      was_split = tm_leaf_insert(TM_ARG reinterpret_cast < LeafNode * >(temp_node), key, value, &result);
    } else {
      //The root is an inner node
      void *temp_node = TM_SHARED_READ_P(root);
      was_split = tm_inner_insert(TM_ARG reinterpret_cast < InnerNode * >(temp_node), d, key, value, &result);
    }
    if (was_split) {
      //The old root was splitted in two parts.
      // We have to create a new root pointing to them
  ...
  __attribute__((transaction_safe)) bool tm_find(TM_ARGDECL const KEY & key, VALUE * value = 0)const {
    ...
    while (d-- != 0) {
      ...
      index = tm_inner_position_for(TM_ARG key, inner->keys, num_keys_1);
    ...
    index = tm_leaf_position_for(TM_ARG key, leaf->keys, num_keys);
    if (index < num_keys) {
  ...
  __attribute__((transaction_safe)) bool tm_del(TM_ARGDECL const KEY & key) {
    InnerNode *inner;
  ...
  __attribute__((transaction_safe)) bool tm_findLastLessThan(TM_ARGDECL const KEY & key, VALUE * value = 0, KEY * out_key = 0)const {
    const void *node = TM_SHARED_READ_P(root);
    unsigned long d = TM_SHARED_READ(depth);
    while (d-- != 0) {
      const InnerNode *inner = reinterpret_cast < const InnerNode * >(node);
      unsigned long num_keys_1 = TM_SHARED_READ(inner->num_keys);
      unsigned int pos = tm_inner_position_for(TM_ARG key, inner->keys, num_keys_1);
    ...
    const LeafNode *leaf = reinterpret_cast < const LeafNode * >(node);
    unsigned long num_keys = TM_SHARED_READ(leaf->num_keys);
    unsigned int pos = tm_leaf_position_for(TM_ARG key, leaf->keys, num_keys);
    ...
    if (pos <= num_keys) {
      ...
      if (pos < num_keys) {
        ...
        if (temp_value) {
          ...
          return tm_findLastLessThan(TM_ARG temp_key_4, value, out_key);
        }
      }
    }
    return false;
...
private:
  ...
  enum NodeType {
    ...
    */
    ...
    static unsigned tm_leaf_position_for(TM_ARGDECL const KEY & key, const KEY * keys, unsigned long num_keys){
      //Simple linear search.Faster for small values of N or M
      unsigned k = 0;
    ...
    // that has the given keys.
    static inline unsigned tm_inner_position_for(TM_ARGDECL const KEY & key, const KEY * keys, unsigned long num_keys){
      //Simple linear search.Faster for small values of N or M
      unsigned k = 0;
      intptr_t temp_key = TM_SHARED_READ(keys[k]);
      while    ((k < num_keys) && ((temp_key < key) || (temp_key == key))) {
    ...
    __attribute__((transaction_safe)) bool tm_leaf_insert(TM_ARGDECL LeafNode * node, KEY & key, VALUE & value, InsertionResult * result) {
      bool was_split = false;
      //Simple linear search
      unsigned long num_keys = TM_SHARED_READ(node->num_keys);
      unsigned i = tm_leaf_position_for(TM_ARG key, node->keys, num_keys);
      if (num_keys >= M) {
        //The node was full.We must split it
        unsigned treshold = (M + 1) / 2;
        LeafNode *new_sibling = new_leaf_node();
        ...
        if (i < treshold) {
          //Inserted element goes to left sibling
          tm_leaf_insert_nonfull(TM_ARG node, key, value, i);
        } else {
          //Inserted element goes to right sibling
          tm_leaf_insert_nonfull(TM_ARG new_sibling, key, value, i - treshold);
        }
        //Notify the parent about the split
        was_split = true;
    ...
    __attribute__((transaction_safe)) static void tm_leaf_insert_nonfull(TM_ARGDECL LeafNode * node, KEY & key, VALUE & value, unsigned index){
    ...
    __attribute__((transaction_safe)) bool tm_inner_insert(TM_ARGDECL InnerNode * node, unsigned long current_depth, KEY & key, VALUE & value, InsertionResult * result){
      ...
      if (num_keys >= N) {
        ...
        if (key < result->key) {
          tm_inner_insert_nonfull(TM_ARG node, current_depth, key, value);
        } else {
          tm_inner_insert_nonfull(TM_ARG new_sibling, current_depth, key, value);
        }
      } else {
        //No split
        tm_inner_insert_nonfull(TM_ARG node, current_depth, key, value);
    ...
    __attribute__((transaction_safe)) void tm_inner_insert_nonfull(TM_ARGDECL InnerNode * node, unsigned long current_depth, KEY & key, VALUE & value){
      //Simple linear search
      unsigned long num_keys = TM_SHARED_READ(node->num_keys);
      unsigned index = tm_inner_position_for(TM_ARG key, node->keys, num_keys);
      //GCC warns that this may be used uninitialized, even though that is untrue.
      ...
      if (current_depth - 1 == 0) {
        ...
        if (temp_node == NULL) {
          ...
          //TODO:MEMORY LEAK ! !!
          TM_SHARED_WRITE_P(node->children[index], temp_node);
        }
        was_split = tm_leaf_insert(TM_ARG reinterpret_cast < LeafNode * >(temp_node), key, value, &result);
      } else {
        ...
        was_split = tm_inner_insert(TM_ARG child, current_depth - 1, key, value, &result);
      }
      if (was_split) {
        if (index == num_keys) {
