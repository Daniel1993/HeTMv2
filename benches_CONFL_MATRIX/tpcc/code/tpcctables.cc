...
/*    __transaction_atomic { */
  ...
  // int ro = 1;
  TM_BEGIN();
  d = findDistrict(TM_ARG warehouse_id, district_id);
  o_id = d->d_next_o_id;
...
__attribute__((transaction_safe)) void TPCCTables::paymentHome(TM_ARGDECL int64_t warehouse_id, int64_t district_id, int64_t c_warehouse_id,
    ...
    Warehouse* w = findWarehouse(TM_ARG warehouse_id);
    // if(local_exec_mode == 1 || local_exec_mode == 3){
    //   float temp_float = TM_SHARED_READ_F(w->w_ytd);
    //   TM_SHARED_WRITE_F(w->w_ytd, temp_float+h_amount);
    //   District* d = findDistrict(TM_ARG warehouse_id, district_id);
    //   float temp_float_2 = TM_SHARED_READ_F(d->d_ytd);
    //   TM_SHARED_WRITE_F(d->d_ytd, temp_float_2+h_amount);
    // } else{
    //   float temp_float = TM_SHARED_READ_F(w->w_ytd);
    //   TM_SHARED_WRITE_F(w->w_ytd, temp_float+h_amount);
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(
  ...
  float temp_float = TM_SHARED_READ_F(c->c_balance);
  TM_SHARED_WRITE_F(c->c_balance, temp_float-h_amount);
  float temp_float_2 = TM_SHARED_READ_F(c->c_ytd_payment);
  TM_SHARED_WRITE_F(c->c_ytd_payment, temp_float_2+h_amount);
...
/*    __transaction_atomic { */
  ...
  for (int64_t d_id = 1; d_id <= District::NUM_PER_WAREHOUSE; ++d_id) {
    ...
    NewOrder* neworder;
    int64_t foundKey = -1;
    if (!neworders_.tm_findLastLessThan(TM_ARG key, &neworder, &foundKey)) {
      neworder = NULL;
    }
    // if (local_exec_mode == 1 || local_exec_mode == 3) {
    ...
    neworders_.tm_del(TM_ARG foundKey);
    // if (local_exec_mode == 1 || local_exec_mode == 3)
    ...
    if (o) {
      ...
      for (int64_t i = 1; i <= o->o_ol_cnt; ++i) {
        ...
        if (line) { // TODO: sometimes I get NULLs here
      ...
      float temp_float = TM_SHARED_READ_F(c->c_balance);
      TM_SHARED_WRITE_F(c->c_balance, temp_float+total);
...
__attribute__((transaction_safe)) static T* insert_tm(
  ...
  // }
  tm_memcpy(copy, &item, sizeof(T));
  tree->tm_insert(TM_ARG key, copy);
  // if(local_exec_mode == 1 || local_exec_mode == 3)
...
__attribute__((transaction_safe)) static T* find_tm(
  ...
  int64_t                                                                                            key
    ...
    T* output = NULL;
    bool out;
    out = tree.tm_find(TM_ARG key, &output);
    // if(local_exec_mode == 1 || local_exec_mode == 3)
...
{
  items_.tm_insert(TM_ARG item->i_id, item);
  // if(local_exec_mode == 1 || local_exec_mode == 3) {
...
/*__attribute__((transaction_safe)) Order* TPCCTables::insertOrder(TM_ARGDECL const Order& order)
{
  ...
  if(local_exec_mode == 1 || local_exec_mode == 3) {
    ...
    orders_by_customer_.tm_insert(TM_ARG key, tuple);
  }
  return tuple;
}*/
...
__attribute__((transaction_safe)) Order* TPCCTables::findLastOrderByCustomer(TM_ARGDECL const int64_t w_id, const int64_t d_id, const int64_t c_id)
{
  ...
  key += ((int64_t)1) << 8;
  bool found;
  found = orders_by_customer_.tm_findLastLessThan(TM_ARG key, &order);
  // if(local_exec_mode == 1 || local_exec_mode == 3)
...
/*__attribute__((transaction_safe)) NewOrder* TPCCTables::insertNewOrder(TM_ARGDECL int64_t w_id, int64_t d_id, int64_t o_id)
{
  ...
  if(local_exec_mode == 1 || local_exec_mode == 3)
    ...
    neworders_.tm_insert(TM_ARG key, neworder);
  return neworder;
}*/
...
NewOrder* TPCCTables::findNewOrder(TM_ARGDECL int64_t w_id, int64_t d_id, int64_t o_id)
{
  ...
  bool out;
  out = neworders_.tm_find(TM_ARG makeNewOrderKey(w_id, d_id, o_id), &output);
  if (out) {
    return output;
  }
  return NULL;
