...
__thread FILE *perThreadReadWriteSet = NULL;
__thread unsigned long countPhases = 0;
__thread unsigned long countTXs = 0;
...
stm_exit_thread(void)
{
  ...
  if (perThreadReadWriteSet) fclose(perThreadReadWriteSet);
...
void
stm_loop_rset(stm_loop_rwset_callback clbk)
{
  int_stm_loop_rset(clbk);
}

void
stm_loop_wset(stm_loop_rwset_callback clbk)
{
  int_stm_loop_wset(clbk);
}
