...
static INLINE void
stm_wt_add_to_rs(stm_tx_t *tx, stm_word_t version, volatile stm_word_t *lock, volatile stm_word_t *addr)
{
  r_entry_t *r;
  ...
  if (stm_has_read(tx, lock) != NULL)
    return /*value*/; // TODO: void function should not return... value is undeclared
...
stm_wt_read(stm_tx_t *tx, volatile stm_word_t *addr)
{
 ...
 restart_no_load:
  if (likely(!LOCK_GET_WRITE(l))) {
    ...
    stm_wt_add_to_rs(tx, version, lock, addr);
...
# endif /* ! IRREVOCABLE_IMPROVED */
  ...
  w = tx->w_set.entries;
  struct timespec tp;
  extern __thread FILE *perThreadReadWriteSet;
  extern __thread unsigned long countPhases;
  extern __thread unsigned long countTXs;
  extern long thread_getId();
  if (!perThreadReadWriteSet) {
    char filename[128];
    sprintf(filename, "STM_RWSET_thr%li.txt", thread_getId());
    perThreadReadWriteSet = fopen(filename, "w");
  }
  clock_gettime(CLOCK_REALTIME, &tp);
  fprintf(perThreadReadWriteSet, "# TX=%lu Tinyclock=%lu time=%lu phase=%lu\n", countTXs++, t, tp.tv_sec*(long)10e9 + tp.tv_nsec, countPhases);
  fprintf(perThreadReadWriteSet, "WS=");
  for (i = tx->w_set.nb_entries; i > 0; i--, w++) {
    fprintf(perThreadReadWriteSet, "%p,", w->addr);
    if (w->next == NULL) {
      /* No need for CAS (can only be modified by owner transaction) */
      ATOMIC_STORE(w->lock, LOCK_SET_TIMESTAMP(t));
    }
  }
  fprintf(perThreadReadWriteSet, "\n");
  r_entry_t *r = tx->r_set.entries;
  fprintf(perThreadReadWriteSet, "RS=");
  for (i = tx->r_set.nb_entries; i > 0; i--, r++) {
    fprintf(perThreadReadWriteSet, "%p,", r->addr);
  }
  fprintf(perThreadReadWriteSet, "\n\n");
  /* Make sure that all lock releases become visible */
