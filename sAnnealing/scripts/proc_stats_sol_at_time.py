...
while i < len(sys.argv):
  ...
  thrds = int(graph_name.split("_")[1].strip())
  solution = float(thrds) # number of threads in millis
  samples[graph_name] = []
  ...
  with open(sys.argv[i]) as f:
    ...
    for l in lines:
      ...
      if new_sample:
        ...
        if sample_done == 0 and float(fields[1]) > solution:
          # s = [graph_name]
          s = []
          s += [float(f) for f in fields]
          # print("len(s):", len(s))
...
for k in samples:
  ...
  proc_data[k] += [s for s in np.percentile(np.transpose(new_s)[3], [0,10,50,90,100])]
